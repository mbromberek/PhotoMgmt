...
tell application "Photos"
	if albNameLst is not false then -- not cancelled 
		repeat with albName in albNameLst
			...
			set nDir to dest & name of album albName
			log "New Directory: " & nDir
			my makeDir(nDir)
			...
			repeat with currImg in imgLst
				...
				if not orig then
					...
					set pExporalbName to my replace_chars(pExporalbName, ".mov", ".mov")
					set pExporalbName to my replace_chars(pExporalbName, ".mp4", ".mov")
					set pExporalbName to my replace_chars(pExporalbName, ".MP4", ".mov")
				...
				--This is needed for videos that start with a period
				set pExportableNmLst to my theSplit(pExporalbName, ".")
				set pNewExt to item (count of items in pExportableNmLst) of pExportableNmLst
				set imgExt to "." & pNewExt
				log "Extension: " & imgExt
				...
				try
					do shell script "mv " & quoted form of POSIX path of (nDir & ":" & pExporalbName) & " " & quoted form of POSIX path of (nDir & ":" & pNewName & imgExt)
					tell application "Finder"
					...
					set name of file (nDir & ":" & pExporalbName as alias) to (pNewName & imgExt)*)
						set modification date of file (nDir & ":" & pNewName & imgExt as string) to pDateTime
					end tell
				on error errStr
					log (pExporalbName & " not found")
				end try
